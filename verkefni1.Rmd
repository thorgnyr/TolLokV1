...
plott3
=======
        xlab("Núvirði íbúðar í þúsund krónum") + ylab("Stærð í fermetrum")
>>>>>>> d9093820b33496bec517087246c782738f4d41ce
```
...
Breytileiki stærðar einbýlishúsa í Miðbæ er
Breytileiki stærðar einbýlishúsa í Laugarnesi/Vogum er
Breytileiki stærðar einbýlishúsa í Árbæ er
=======
...
###Miðbær
```{r}
einbyli_1 <- filter(einbyli, matssvaedi=="Midbaer")
mean(einbyli_1$ibm2)
var(einbyli_1$ibm2)
```
###Laugarnes / vogar
```{r}
einbyli_2 <- filter(einbyli, matssvaedi=="LaugarnesVogar")
mean(einbyli_2$ibm2)
var(einbyli_2$ibm2)
```
###Árbær
```{r}
einbyli_3 <- filter(einbyli, matssvaedi=="Arbaer")
mean(einbyli_3$ibm2)
var(einbyli_3$ibm2)
```
>>>>>>> 71574c0fcc08625eb4caa3a9aaaf6d4c04b00a9a
...
```{r}
litlar <- filter(ibudir, ibm2>=50 & ibm2 <=60)
ggplot(data = litlar, aes(matssvaedi, nuvirdi))+
  ## How to plot
        geom_boxplot(alpha=0.1)+
        geom_jitter(alpha = 0.3, color = "red")+
  ## X and Y labels
        xlab("Matssvæði") + ylab("Virði lítilla íbúða í þús. krónum")+
